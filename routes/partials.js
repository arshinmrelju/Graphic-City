...
router.get('/partials/capabilities-services', (req, res) => {
  const html = services.map((s, i) => `<article class="border-t border-stone-200 dark:border-stone-800 py-10 md:py-14 first:border-t-0" x-data="{ expanded: false }" data-reveal>
...
router.get('/partials/about-story', (req, res) => {
  res.send(`<section class="px-6 md:px-12 lg:px-20 py-24 md:py-32 md:min-h-screen flex items-center bg-stone-50 dark:bg-stone-950/50" data-reveal><div class="max-w-6xl mx-auto w-full"><div class="grid md:grid-cols-2 gap-16 md:gap-20">
...
router.get('/partials/about-process', (req, res) => {
  ...
  res.send(`<section class="px-6 md:px-12 lg:px-20 py-24 md:py-32 bg-core-white dark:bg-core-black" data-reveal><div class="max-w-7xl mx-auto w-full"><p class="font-body text-xs font-medium tracking-wider uppercase text-stone-400 dark:text-stone-500 mb-4">How We Work</p><h2 class="font-display font-light text-h2 md:text-display-sm text-core-black dark:text-core-white -tracking-tight mb-4">A process built for partnership.</h2><p class="font-body text-base md:text-lg text-stone-500 dark:text-stone-400 max-w-xl mb-16">Not a rigid formula. A flexible framework shaped around each project.</p>
    <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-8">${steps.map((s, i) => `<div class="reveal-item"><p class="font-display text-4xl md:text-5xl font-light text-stone-200 dark:text-stone-800 mb-4">${s.num}</p><h3 class="font-display text-xl text-core-black dark:text-core-white mb-3">${s.title}</h3><p class="font-body text-sm text-stone-500 dark:text-stone-400 leading-relaxed text-balance">${s.desc}</p></div>`).join('')}</div></div></section>`);
});

router.get('/partials/about-numbers', (req, res) => {
  res.send(`<section class="px-6 md:px-12 lg:px-20 py-24 md:py-32 bg-core-black text-core-white" data-reveal><div class="max-w-7xl mx-auto w-full"><p class="font-body text-xs font-medium tracking-wider uppercase text-stone-500 mb-4">By the Numbers</p><h2 class="font-display font-light text-h2 md:text-display-sm -tracking-tight mb-16">The sum of our parts.</h2>
...
router.get('/partials/about-manifesto', (req, res) => {
  res.send(`<section class="px-6 md:px-12 lg:px-20 py-24 md:py-32 bg-stone-50 dark:bg-stone-950/50" data-reveal><div class="max-w-4xl mx-auto"><p class="font-body text-xs font-medium tracking-wider uppercase text-stone-400 dark:text-stone-500 mb-4 reveal-item">Manifesto</p><div class="space-y-8 reveal-item"><p class="font-display text-h3 md:text-h1 font-light text-core-black dark:text-core-white leading-tight -tracking-tight">Design is not decoration.</p><p class="font-display text-h3 md:text-h1 font-light text-core-black dark:text-core-white leading-tight -tracking-tight">It is a conversation between what something wants to be and how it is perceived.</p><p class="font-display text-h3 md:text-h1 font-light text-core-black dark:text-core-white leading-tight -tracking-tight">We choose clarity over cleverness. Substance over spectacle. Intent over impulse.</p><p class="font-display text-h3 md:text-h1 font-light text-core-black dark:text-core-white leading-tight -tracking-tight">Every project deserves a point of view. Not a formula.</p></div></div></section>`);
});
